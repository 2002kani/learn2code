...
function getTimelineEvents(
  ...
  if (
    articleRevision.publishProcessStopped &&
    articleRevision.publishProcessStoppedAt
  ) {
    events.push({
      title: i18next.t("Process stopped"),
      createdAt: DateTimeLuxonHelpers.convertUtcToDateTime(
        articleRevision.publishProcessStoppedAt
      ),
  ...
  approvals
    ...
    .forEach((x) =>
      events.push({
        title: `${x.approver.fullName} ${
          x.isApproved ? i18next.t("has approved") : i18next.t("has rejected")
        }`,
        createdAt: DateTimeLuxonHelpers.convertUtcToDateTime(x.approvedAt),
        icon: x.isApproved ? (
          <qmBaseIcons.CheckSquare color="success" />
        ) : (
          <qmBaseIcons.Alert color="danger" />
        ),
        container: x.comment != "" ? x.comment : undefined,
      })
    );
  if (
    articleRevision.isPublishProcessStarted &&
    articleRevision.lastUpdatedAt
  ) {
    events.push({
  ...
  events.push({
    title: `${articleRevision.createdBy?.fullName} ${i18next.t(
      "has created this revision"
    )}`,
    createdAt: DateTimeLuxonHelpers.convertUtcToDateTime(
      articleRevision.createdAt
    ),
